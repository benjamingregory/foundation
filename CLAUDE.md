...
| Image generation | **Recraft** | Raster and real **vector/SVG** output. Asked directly for a logo, a domain-specific mark, or an illustration (any app, once normalized); also the tool `brandkit` and `imagegen-frontend-web` depend on (`website/`). Optional. See § Recraft. | MCP server, declared in [.mcp.json](.mcp.json) |
...
| **`brandkit`** | **At init** — [project-init](.claude/skills/project-init/SKILL.md) Step 10 | The brand doesn't exist yet — logo system, identity board, visual world. It runs *before* the tokens are written, which is why it's a numbered init step and not a gate: once tokens exist its output competes with them. Its palette and type feed `website/`'s tokens, and its logo goes through the SVG path below. Needs Recraft, and spends credits, so it's opt-in with explicit approval. |
...
**Recraft generates images, and — the reason it's here — real SVG.** [recraft.ai](https://www.recraft.ai/docs), declared as an MCP server in [.mcp.json](.mcp.json) alongside Playwright. Two distinct uses, and the second is the common one:

1. **As the dependency of two skills.** `brandkit` and `imagegen-frontend-web` are the only skills here that can't degrade — every other one still produces *something* when a dependency is missing, but an image skill with no image tool produces nothing at all. Both are `website/`-scoped. `corepack pnpm doctor` reports the server in the loop-gates block for that reason.
2. **Directly, to generate a vector asset.** "Generate an SVG of X" is the highest-value thing to ask it for, and unlike a raster comp it is **not** `website/`-only — a normalized SVG inherits `currentColor`, so it survives the token lock and dark mode in `web/` and `admin/` too.

**Generating an SVG.** The rules that keep it from colliding with the icon set and the token lock:

- **Only for a mark no icon library has** — a product logo or wordmark, a domain-specific glyph, an empty-state or error illustration, a decorative brand shape. **Never a generic UI icon.** Settings, trash, chevron, spinner: those belong to the project's icon library, and `design-taste-frontend` § 3.C bans hand-rolled icon paths for exactly this reason. This is the `pick-ui-library` rule in another medium — don't generate a primitive that already exists.
- **Ask for vector explicitly.** Only the `*-vector` models emit SVG; the raster models return a PNG no matter how you word the prompt. Roughly $0.08 per V4.1 Vector image, $0.30 for the Pro tier. If the mark already exists as a raster, **vectorize** it instead for $0.01 — far cheaper and it preserves the actual artwork rather than reinterpreting it.
- **Normalize before it lands in `web/` or `admin/`.** Generated SVG arrives with baked hex fills, and an inline hex is hard stop #4. Strip them so the shape inherits the theme, then compress:

  ```js
  // svgo.config.mjs — SVGO's --config takes a .js/.mjs/.cjs path, not inline JSON
  export default {
    multipass: true,
  ...
  ```bash
  npx svgo in.svg -o out.svg --config svgo.config.mjs
  ```

  **Check the result by eye — this flattens every fill.** Verified on a two-color mark, the `#1a1a1a` body and the `#FF6600` accent both came out as `currentColor`, which is right for a monochrome glyph and wrong for anything with a second color. A multi-color mark needs its non-primary fills mapped back to `var(--token)` by hand. A mark that genuinely needs its own color is a token lifted into [globals.css](web/app/globals.css) deliberately, not a hex left in the asset.
- **Where it goes.** `public/` for a static asset referenced by `next/image`; an inline component in `components/ui/` when it needs to take `className` and pick up `currentColor` from its parent. Inline is what makes theming work — an `<img>` pointing at an SVG file can't inherit anything.
- **Give it an accessible name.** Decorative marks get `aria-hidden="true"`; anything carrying meaning gets `role="img"` and a `<title>`. Hallmark's a11y gate will catch this, but catching it here is cheaper.
- **Never as a fake screenshot.** `design-taste-frontend` § 9.F bans div-based fake product UI as the single biggest LLM design tell. A generated one is the same claim with better rendering — worse, not better, because it's more convincing. Use a real screenshot or nothing.
...
| `brandkit` / everything else | **Before the loop, not inside it.** brandkit designs a brand that doesn't exist yet; every other design skill here assumes tokens already exist. Its slot is project-init Step 10. If it fires after the tokens are written, that's a misfire — a rebrand is a deliberate decision, not a skill invocation. |
