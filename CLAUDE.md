...
| `strategize` | Framing the problem before solutions exist; sizing the opportunity | G1 |
| `philosopher` | Sitting with a problem instead of rushing it. A cognitive mode, not a step | G1 |
...
| `specify` | Design→engineering handoff: specs, asset packages | G3 |
| `blueprint` | Mapping and redesigning the systems behind an experience | G2/G3 |
...
| **Four review passes.** `/audit`, `evaluate`, `review-animations`, and `/design-review` all review a finished surface. | **Different eyes; pick by question.** `/audit` is technical static analysis (a11y, performance, theming). `evaluate` is quantitative UX assessment against heuristics. `review-animations` reads motion code against emil's standards. `/design-review` drives a real browser at four viewports and is the only one that sees what actually renders. Run `/design-review` always; the others when the change touches their subject. |
