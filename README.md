...
- **[emilkowalski/skills](https://github.com/emilkowalski/skills)** (MIT) settles the motion — four of its seven. `emil-design-eng` implements against this project's Motion conventions; `find-animation-opportunities` decides where motion earns its place and what to leave still; `review-animations` reviews what shipped; `pick-ui-library` runs before any new UI dependency, so nobody hand-rolls a toast that `components/ui/` already has.
- **[design-taste-frontend](https://github.com/Leonxlnx/taste-skill)** (MIT) owns page-level work in `website/` and nowhere else — it scopes itself out of dashboards, product UI, and data tables, which is a description of `web/` and `admin/`.
