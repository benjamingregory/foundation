...
- **`impeccable`** supplies the vocabulary and the targeted passes. Its `frontend-design` skill is the reference for typography, OKLCH color, spacing, motion curves, interaction, responsive behavior, and UX writing; its commands are ordered, not interchangeable — `/critique` first (a hierarchy finding can invalidate detail work), `/polish` once critique findings are fixed, `/animate` last, `/audit` before review and whenever you inherit UI you didn't write.
