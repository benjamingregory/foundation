...
**Precedence.** Process skills set the approach; design and domain tools (`hallmark`, the seven `emilkowalski/skills`, the `nextjs-frontend-expert` / `mastra-workflow-architect` subagents) execute inside the plan those skills produce. `/issues` and `/work` are GitHub-Issues commands kept for projects that track work there instead — this project uses bd.
