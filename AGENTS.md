...
**Recraft** is declared as an MCP server in [.mcp.json](.mcp.json) and is the image tool behind `brandkit` and `imagegen-frontend-web` — the only two skills that produce nothing at all without their dependency. It authenticates over OAuth (no secret in the file), is optional and `website/`-scoped, and bills against a different balance than its REST API does. `corepack pnpm -C web doctor` reports it. Nothing in `web/` calls it at runtime; see CLAUDE.md § Recraft.
