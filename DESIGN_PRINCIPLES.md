...
`design-taste-frontend` has three siblings from the same repo that are commonly installed user-level and are **not** part of this loop: `brandkit` (a pre-project brand exercise, before `website/`'s tokens exist), `imagegen-frontend-web` (optional reference comps for an unsettled `website/` page), and `redesign-existing-projects` (never — it opens by replacing the font and collapsing to one accent color). The first two generate images and need the Recraft MCP server; both are `website/`-scoped, and `brandkit`'s slot is project-init Step 10, before the tokens exist. Generated imagery is an asset, never a license to introduce a palette.

**Asking Recraft for an SVG directly is in scope everywhere**, unlike those two skills — a normalized SVG inherits `currentColor`, so it holds the theme in `web/` and `admin/` too. Only for a mark no icon library has: a logo, a wordmark, a domain-specific glyph, an empty-state illustration. Never a generic UI icon — those come from the icon library, and generating one is the same mistake as hand-rolling a primitive that already exists in `components/ui/`. Strip the baked hex fills to `currentColor` before the file lands, or it violates the token lock. See CLAUDE.md § The tasteskill family and § Recraft.
